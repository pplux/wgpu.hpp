...
namespace wgpu {
    ...
    typedef WGPUExternalTexture ExternalTexture;
    ...
    DESCRIPTOR(CopyTextureForBrowserOptions)
        PROP(flipY)
        PROP(needsColorSpaceConversion)
        PROP(srcAlphaMode)
        PROP(srcTransferFunctionParameters)
        PROP(conversionMatrix)
        PROP(dstTransferFunctionParameters)
        PROP(dstAlphaMode)
    END

    SUBCLASS(DawnCacheDeviceDescriptor, WGPUSType_DawnCacheDeviceDescriptor)
        PROP(isolationKey)
    END

    SUBCLASS(DawnEncoderInternalUsageDescriptor, WGPUSType_DawnEncoderInternalUsageDescriptor)
        PROP(useInternalUsages)
    END

    SUBCLASS(DawnInstanceDescriptor, WGPUSType_DawnInstanceDescriptor)
        LIST(additionalRuntimeSearchPaths,additionalRuntimeSearchPathsCount)
    END

    SUBCLASS(DawnTextureInternalUsageDescriptor, WGPUSType_DawnTextureInternalUsageDescriptor)
        PROP(internalUsage)
    END

    SUBCLASS(DawnTogglesDeviceDescriptor, WGPUSType_DawnTogglesDeviceDescriptor)
        LIST(forceEnabledToggles,forceEnabledTogglesCount)
        LIST(forceDisabledToggles,forceDisabledTogglesCount)
    END
    ...
    SUBCLASS(ExternalTextureBindingEntry, WGPUSType_ExternalTextureBindingEntry)
        PROP(externalTexture)
    END

    SUBCLASS(ExternalTextureBindingLayout, WGPUSType_ExternalTextureBindingLayout)
    END
    ...
    SUBCLASS(PrimitiveDepthClampingState, WGPUSType_PrimitiveDepthClampingState)
        PROP(clampDepth)
    END
    ...
    SUBCLASS(SurfaceDescriptorFromWindowsCoreWindow, WGPUSType_SurfaceDescriptorFromWindowsCoreWindow)
        PROP(coreWindow)
    END
    ...
    SUBCLASS(SurfaceDescriptorFromWindowsSwapChainPanel, WGPUSType_SurfaceDescriptorFromWindowsSwapChainPanel)
        PROP(swapChainPanel)
    END

    SUBCLASS(SurfaceDescriptorFromXlibWindow, WGPUSType_SurfaceDescriptorFromXlibWindow)
        PROP(display)
        PROP(window)
    END
    ...
    CLASS(DeviceProperties)
        ...
        PROP(vendorID)
        PROP(adapterType)
        PROP(textureCompressionBC)
        PROP(textureCompressionETC2)
        PROP(textureCompressionASTC)
        PROP(shaderFloat16)
        PROP(pipelineStatisticsQuery)
        PROP(timestampQuery)
        PROP(multiPlanarFormats)
        PROP(depthClamping)
        PROP(depth24UnormStencil8)
        PROP(depth32FloatStencil8)
        PROP(invalidFeature)
        PROP(dawnInternalUsages)
        PROP(dawnNative)
        PROP(limits)
    END
